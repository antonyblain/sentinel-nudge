...
_Section revue par l'expert accessibilité et l'expert UX/UI (2026-04-11)._
...
## 9. Addendum v1.2 — Travaux P5 (stub)

> **Statut stub** — cette section ouvre le document à l'intégration progressive des cas d'usage et décisions structurantes issus de la phase P5 (fiabilisation M7 + cycle UC P0). Le contenu détaillé sera porté par des PR incrémentales (une par UC, une par ARB, une par ADR) conformément à LL-032, afin d'éviter un bump monolithique. Les documents référencés ci-dessous font foi en l'état jusqu'à leur intégration formelle dans §2 à §8.

### 9.1 Cas d'usage UC-01 à UC-15

Les 15 cas d'usage issus du **post-mortem M7** (`docs/gouvernance/gouvernance-pv-postmortem-m7-v1.0.md`) et du **plan de tests manuels consolidé** (`docs/p5-recette/plan-tests-manuels-consolide-v1.0.md`) sont traités comme annexe opposable de ce SFD jusqu'à intégration détaillée. Découpage P0 / P1 / P2 :

| UC            | Titre                                                          | Priorité    | Référence actuelle                                                                                                                        |
| ------------- | -------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| UC-01         | Login multi-étape (Google, Microsoft)                          | P0 v1       | `docs/p5-decisions/p5-minidat-tache-068-uc01-login-multi-etape-v1.1.md` + `docs/p5-recette/p5-matrice-compatibilite-providers-m7-v1.0.md` |
| UC-02         | Password managers (KeePass, KeePassXC, Bitwarden, Vaultwarden) | P0 v1       | Post-mortem M7 §UC-02                                                                                                                     |
| UC-03         | Iframes same-origin                                            | P0 v1       | `docs/p5-decisions/p5-minidat-tache-070-uc03-iframes-v1.0.md`                                                                             |
| UC-04         | Iframes cross-origin                                           | P0 v1       | `docs/p5-decisions/p5-minidat-tache-070-uc03-iframes-v1.0.md`                                                                             |
| UC-05         | Toggle show/hide password                                      | P0 v1       | `docs/p5-decisions/p5-minidat-tache-072-toggle-show-hide-v1.1.md`                                                                         |
| UC-06         | Inputs dynamiques (SPA, React/Vue mount tardif)                | P0 v1       | Post-mortem M7 §UC-06                                                                                                                     |
| UC-07 à UC-15 | Couverture défensive complémentaire                            | P1/P2 v1.1+ | Post-mortem M7 §UC-07 à §UC-15                                                                                                            |

**Intégration détaillée** — chacun de ces UC sera porté en PR dédiée (tâches T-175 E2E UC-01 à UC-05 ; T-068 recette manuelle Google+MS ; etc.) et rapatrié au corps du SFD (§2 à §8) au fur et à mesure.

### 9.2 Arbitrages structurants ARB-061-01/02/03

Trois arbitrages du mini-DAT `p5-minidat-tache-061-heartbeat-m7-v1.1.md` ont valeur structurante sur la spécification fonctionnelle M7 :
...
Deux ADR `Accepted` s'appliquent transversalement à tous les modules du SFD :

| ADR     | Titre                  | Statut              | Portée SFD                                                                                                                         |
| ------- | ---------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| ADR-001 | SW-BOOT-CONTRACT       | Accepted 2026-04-17 | §2 tous modules — tout handler avec prérequis storage implémente le contrat de boot (lire → valider → régénérer/migrer → logger)   |
| ADR-002 | CROSS-LIFECYCLE-INTENT | Accepted 2026-04-17 | §2 tous modules — toute action traversant dormance/redirect/réinjection est persistée en storage avec TTL, consommée à destination |

**Intégration détaillée** — portée par l'audit `docs/p5-decisions/p5-audit-modules-adr-compliance-v1.0.md` et les correctifs M2/M3/M5/M6/M9/M17 (PR #19 à #25, déjà mergées).

### 9.4 Prochaines évolutions attendues (roadmap SFD)

| Évolution                                                         | Priorité    | Tâche BACKLOG       |
| ----------------------------------------------------------------- | ----------- | ------------------- |
